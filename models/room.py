    ...
    if 'students' in vals:
        ...
        for command in vals['students']:
            ...
            if cmd_type == 4:
                ...
                current_student_ids = set(command[1])
            elif cmd_type == 3:
        ...
        if len(current_student_ids) > room.capacity:
            raise osv.except_osv(_('Error!'), _('No se puede agregar: la sala ya alcanzo su capacidad.'))
    return super(Room, self).write(cr, uid, vals, context=context)
